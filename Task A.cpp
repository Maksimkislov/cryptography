...
int main() {
    ...
    for (unsigned long long i = 0; i < str.size(); ++i) {
        unsigned long long t = char_to_number(str[i]);
        arr.push_back(t);
    }
    ...
    while (dec != zero) {
        ...
        divLongToShort(dec, p, rem, 10);
        res64.push_back(rem);
    }

    for (unsigned long long i = 0; i < res64.size(); ++i) {
        unsigned long long a, b;
        a = Pow(g, 2, p);
        b = Pow(k, 2, p) * res64[i] % p;
        cout << a << " " << b << "\n";
    }
